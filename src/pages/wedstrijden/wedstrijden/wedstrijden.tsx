...
export function wedstrijdGeweest(dateStringArg: string) {
  let dateString;
  if (dateStringArg.includes("en")) {
    dateString = dateStringArg.split("en")[1]!;
  } else {
    dateString = dateStringArg;
  }
  for (let month in dateMap) {
    if (dateString.toLowerCase().includes(month)) {
      dateString = dateString.replace(month, dateMap[month]!);
    }
  }
    ...
    <div>
      ...
      <div id="eigenWedstrijden">
        {TextWedstrijden.wedstrijden.map((wedstrijd) => (
          <EigenWedstrijd
            key={wedstrijd.naam + wedstrijd.datum}
            naam={wedstrijd.naam}
            datum={wedstrijd.datum}
            logo={wedstrijd.logo}
            info_kort={wedstrijd.info_kort}
            path={wedstrijd.path}
            oud={wedstrijdGeweest(wedstrijd.datum)}
          />
        ))}
      </div>
