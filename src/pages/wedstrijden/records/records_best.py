...
output = {"man": {"outdoor": [], "indoor": []}, "vrouw": {"outdoor": [], "indoor": []}}
    ...
    for gesl in onderdelen:
        for cat in onderdelen[gesl]:
            ...
            for event in onderdelen[gesl][cat]:
                ...
                best_records = event_rows[best_idx.all(axis=1)].drop_duplicates(
                    subset="Naam"
                )
                best = best_records.sort_values(by="Score", ascending=lower_better)[:5]
                best_dict = best.to_dict(orient="index")

                output[gesl][cat].append(
                    {"onderdeel": event, "prestaties": convert_dict(best_dict)}
                )
